...
pi_delta_scatterplot <- function(robust_estimate, save_pdf = FALSE,
  ...
  x_or_p <- mean(abs(omega_or_norm[index_pos_or]) * fs_coeffs_full[index_pos_or]) / 
                        mean(abs(omega_or_norm[index_pos_or]))
  x_or_n <- mean(abs(omega_or_norm[!index_pos_or])*fs_coeffs_full[!index_pos_or]) / 
                        mean(abs(omega_or_norm[!index_pos_or]))
  y_or_p <- mean(abs(omega_or_norm[index_pos_or])*rf_coeffs_full[index_pos_or]) / 
                        mean(abs(omega_or_norm[index_pos_or]))
  y_or_n <- mean(abs(omega_or_norm[!index_pos_or])*rf_coeffs_full[!index_pos_or]) / 
    mean(abs(omega_or_norm[!index_pos_or]))
  
  x_p <- mean(abs(omega_rob_norm[index_pos_rob])*fs_coeffs_post[index_pos_rob]) / 
                        mean(abs(omega_rob_norm[index_pos_rob]))
  x_n <- mean(abs(omega_rob_norm[!index_pos_rob])*fs_coeffs_post[!index_pos_rob]) / 
                        mean(abs(omega_rob_norm[!index_pos_rob]))
  y_p <- mean(abs(omega_rob_norm[index_pos_rob])*rf_coeffs_post[index_pos_rob]) / 
                        mean(abs(omega_rob_norm[index_pos_rob]))
  y_n <- mean(abs(omega_rob_norm[!index_pos_rob])*rf_coeffs_post[!index_pos_rob]) / 
                        mean(abs(omega_rob_norm[!index_pos_rob]))
  ...
  if (save_pdf){
    ...
    if (nchar(file) >= 4){
      if (substring(file, nchar(file) - 3) == ".pdf") {
        dir_1 <- paste(substring(file, 1, nchar(file) - 3), '_original', '.pdf', sep = '')
      } else {
    ...
    plot(x = fs_coeffs_full, y = rf_coeffs_full, ylim = range_rf,xlim = range_fs, 
         pch = 1, frame = FALSE, bg=NULL, col=1 + as.numeric(omega_or_norm > 0), 
         cex = abs(omega_or_norm),
         xlab = 'First Stage Coefficients', ylab = 'Reduced Form Coefficients')
    abline(a = weighted.mean(rf_coeffs_full - tau_or*fs_coeffs_full, w = abs(omega_or_norm)), 
           b = tau_or, lty = 2, col = 'grey')
    points(cbind(c(x_or_p,x_or_n), c(y_or_p,y_or_n)), 
            cex = 2,pch = 2, col = 'blue')
    legend(x = x_p, y = y_n - 0.3, legend = substitute(paste(hat(tau)[TSLS], 
           ' = ', tau_or, ', ', hat(se)(hat(tau)[TSLS]), ' = ', se_or_cor),
    ...
    plot(x=fs_coeffs_full, y=rf_coeffs_full, ylim = range_rf,xlim = range_fs, 
         ...
         cex = abs(omega_or_norm),
         xlab = 'First Stage Coefficients', ylab = 'Reduced Form Coefficients')
    abline(a = weighted.mean(rf_coeffs_full - tau_or*fs_coeffs_full, 
           w = abs(omega_or_norm)), b =tau_or, lty = 2, col = 'grey')
  ...
  if (save_pdf){
    ...
    plot(x = fs_coeffs_post, y = rf_coeffs_post, pch = 1, 
         ylim = range_rf, xlim = range_fs, frame = FALSE,
         bg = NULL , col = 1 + as.numeric(omega_rob_norm > 0),
         cex = abs(omega_rob_norm),
         xlab = 'First Stage Coefficients', ylab = 'Reduced Form Coefficients')
    abline(a = weighted.mean(rf_coeffs_post - tau_rob*fs_coeffs_post,
           w = abs(omega_rob_norm)), b =tau_rob,lty = 2, col = 'grey')
    points(cbind(c(x_p,x_n),c(y_p,y_n)),cex = 2,pch = 2, col = 'blue' )
    legend(x = x_p + 0.3, y = y_n - 0.3, legend = substitute(paste(hat(tau)[rob],
           ' = ', tau_rob, ', ', hat(se)(hat(tau)[rob]), ' = ', se_rob_cor),
           list(tau_rob = round(tau_rob, 2), se_rob_cor = round(se_rob_cor, 2))),
           cex=1)
